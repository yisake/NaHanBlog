...
@voice.route("/voice/create2", methods=['GET', 'POST'])
@login_required
def create2():
    """ Copy from voice create, and upload content with simditor editor
    """
    if request.method == 'GET':
        return render_template('voice/create.html',
                               title=gettext('Create Topic'),
        ...
        title = request.form['title']
        content = request.form['content']
        sim_content=request.form['sim-content']
        node_id = int(request.form['node'])
        user_id = current_user.id
        print sim_content
...
def edit(tid):
    ...
    if request.method == 'GET':
        ...
        '''
        @change: change form content to sim-content
        @author: 皇甫 
        '''
        #new_content = request.form['content']
        new_content = request.form['sim-content']
        if not new_content:
...
'''
@author:  皇甫
@note: 增加simditor插件的后台接口，存储文件
'''
@voice.route('/voice/simupload/' ,methods=['GET' , 'POST' ])
@login_required
def simupload():
    print request.form
    print request.files
    ...
    user_path=os.path.normcase(os.getcwd()+'/nahan/static/uploads/' +str(current_user.id)+'/' )
    print user_path
    if os.path.exists(user_path)==False:
        os.mkdir(user_path)
    
    def generate_random_file_name():
        return ''.join(random.sample(string.digits+string.ascii_letters,10))+upload_file.filename[upload_file.filename.find('.' ):]
    
    for i in range(0,10):
        random_filename=generate_random_file_name()
        if os.path.exists(os.path.join(user_path, random_filename))==False:
            break
        if i==9:
            return jsonify(success=False)
        
    upload_file.save(os.path.join(user_path , random_filename))
    return jsonify(success=True , file_path='http://127.0.0.1:5000/static/uploads/'+str(current_user.id)+'/'+random_filename)
